...
//TODO: use a real RNG.
void randbyte(uint8_t *b)
{
    *b = 0;
    return;
}

//A nibble (4 bits) to ASCII hex.
char n2a(uint8_t n)
{
    char hexascii = 0;
...
void remask(uint8_t s[16], uint8_t m1, uint8_t m2, uint8_t m3, uint8_t m4,
...
void init_masked_round_keys()
{
    ...
    for (i = 0; i < 10; i++)
    {
        remask(RoundKey_masked[i], Mask[6], Mask[7], Mask[8], Mask[9], Mask[4],
...
//AES main function by Si Gao and David McCann.
/******************************************************************/
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        {
            output[j] = input[j];
            putchar(n2a(output[j] >> 4));
            putchar(n2a(output[j] & 0x0F));
        }
        putchar('\n');
    }
